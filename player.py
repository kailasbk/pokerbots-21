	...
	def get_actions(self, game_state, round_state, active):
		...
		for i in range(NUM_BOARDS):
			if AssignAction in legal_actions[i]:
				...
				if winProb >= .8 and RaiseAction in legal_actions[i]:
					if BIG_BLIND <= 3 * BIG_BLIND and 3 * BIG_BLIND <= stack:
						my_actions[i] = RaiseAction(3 * BIG_BLIND)
					else:
						my_actions[i] = RaiseAction(stack)
